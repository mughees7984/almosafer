...
const TopHeader: React.FC<{ onMenuToggle: () => void; isMenuOpen: boolean }> = ({ onMenuToggle, isMenuOpen }) => {
  return (
    <header className={headerBg} aria-label="Top header">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex-shrink-0">
            <img 
              ...
              height={150} 
              alt="Logo" 
              className="h-12 w-auto sm:h-16 md:h-20 lg:h-auto lg:w-[150px]"
            />
          </div>
          ...
          <button
            ...
            className="lg:hidden text-white hover:text-gray-200 transition p-2 -mr-2"
...
const SecondaryNav: React.FC = () => {
  return (
    <nav className={`${headerBg} hidden lg:block`} aria-label="Main navigation">
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex space-x-8 py-3">
          {navItems.map(({ key, Icon, label }) => (
            <button
...
const MobileMenu: React.FC<{ isOpen: boolean; onClose: () => void }> = ({ isOpen, onClose }) => {
  ...
  return (
    <>
      ...
      {/* Menu Panel */}
      <div className={`fixed top-0 right-0 bottom-0 w-[280px] ${headerBg} z-[9999] lg:hidden overflow-y-auto`}>
              ...
              <div className="space-y-1">
                {navItems.map(({ key, Icon, label }) => (
                  <button
                    ...
                    type="button"
                    onClick={onClose}
                    className="flex items-center space-x-3 text-white hover:bg-white/10 transition w-full py-2.5 px-3 rounded-md text-left text-sm"
                    aria-label={label}
                  >
                    <Icon width={18} height={18} aria-hidden="true" className="flex-shrink-0" />
              ...
              <div className="space-y-1">
                ...
                >
                  ...
                  <div className="flex items-center space-x-1">
                    <span aria-hidden="true" className="text-sm">SAR</span>
                    <svg
                      className="w-4 h-4"
                      fill="none"
                      stroke="currentColor"
                ...
                <a
                  href={WHATSAPP_LINK}
                  className="text-white hover:bg-white/10 transition flex items-center space-x-3 py-2.5 px-3 rounded-md text-sm"
                  target="_blank"
                  rel="noopener noreferrer"
                  aria-label={`Contact via WhatsApp ${PHONE}`}
                  onClick={onClose}
                >
                  <Phone size={16} className="stroke-current flex-shrink-0" aria-hidden="true" />
                  <span>{PHONE}</span>
                </a>
              </div>
            </div>
